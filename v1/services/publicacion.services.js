...
export const obtenerPublicacionesService = async () => {
    const publicaciones = await Publicacion.find().populate("tipoObra").populate("ganador", "nombre email");
...
export const modificarPublicacionService = async (id, dataActualizada) => {
    ...
    const publicacionModificada = await Publicacion.findByIdAndUpdate(id, dataActualizada, { returnDocument: "after" });
