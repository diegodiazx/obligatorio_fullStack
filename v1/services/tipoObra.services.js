...
export const crearTipoObraService = async (data) => {
  if (
    await TipoObra.findOne({
      //tiene que coincidir el string completamente, no parcialmente
      nombre: { $regex: `^${data.nombre}$`, $options: "i" },
    })
  ) {
    const error = new Error("El tipo de obra ya existe");
    error.status = 400;
    throw error;
  }
...
export const eliminarTipoObraService = async (id) => {
  ...
  if (publicacionesAsociadas.length > 0) {
    const error = new Error(
      "No se puede eliminar el tipo de obra porque tiene publicaciones asociadas",
    );
    error.status = 400;
    error.details = {
      id: id,
      publicacionesAsociadas: publicacionesAsociadas.map((pub) => pub._id),
    };
    throw error;
  }
...
export const modificarTipoObraService = async (id, datosActualizados) => {
  ...
  const tipoObraModificada = await TipoObra.findByIdAndUpdate(
    id,
    datosActualizados,
